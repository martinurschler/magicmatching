...
def draw_single_line(img):
    ...
    """
    distance_between_pts = 0
    MIN_DIST = 5
    x1 = x2 = y1 = y2 = None
    min_dim = min(img.shape)
    while distance_between_pts < MIN_DIST:
        ...
        p2 = np.array([[x2, y2]])
        distance_between_pts = np.linalg.norm(p1 - p2)

    background_color = int(np.mean(img))
    col = get_random_color(background_color)
    thickness = max(1, np.random.randint(min_dim * 0.01, min_dim * 0.02))
    cv2.line(img, (x1, y1), (x2, y2), col, thickness)
    #print("line between", (x1, y1), (x2, y2))
    # print("painted image setup:", np.min(img), np.max(img))
...
def draw_polygon(img, max_sides=8):
    ...
    points = np.array([[int(x + max(np.random.rand(), 0.4) * rad * math.cos(a)),
                        int(y + max(np.random.rand(), 0.4) * rad * math.sin(a))]
                       for a in angles])
...
def draw_multiple_polygons(img, max_sides=8, nb_polygons=30, **extra):
    ...
    for i in range(nb_polygons):
        ...
        angles = [slices[i] + np.random.rand() * (slices[i+1] - slices[i])
                  for i in range(num_corners)]
        new_points = [[int(x + max(np.random.rand(), 0.4) * rad * math.cos(a)),
                       int(y + max(np.random.rand(), 0.4) * rad * math.sin(a))]
                      for a in angles]
...
def draw_stripes(img, max_nb_cols=13, min_width_ratio=0.04,
    ...
    thickness = max(1, np.random.randint(min_dim * 0.01, min_dim * 0.015))
...
def draw_cube(img, min_size_ratio=0.2, min_angle_rot=math.pi / 10,
    ...
    scaling = np.array([[scale_interval[0] +
                         np.random.rand() * scale_interval[1], 0, 0],
                        [0, scale_interval[0] +
                         np.random.rand() * scale_interval[1], 0],
                        [0, 0, scale_interval[0] +
                         np.random.rand() * scale_interval[1]]])
    trans = np.array([img.shape[1] * trans_interval[0] +
                      np.random.randint(-img.shape[1] * trans_interval[1],
                                           img.shape[1] * trans_interval[1]),
                      img.shape[0] * trans_interval[0] +
                      np.random.randint(-img.shape[0] * trans_interval[1],
    ...
    for i in [0, 1, 2]:
        cv2.fillPoly(img, [cube[faces[i]].reshape((-1, 1, 2))],
                    col_face)
    thickness = max(1, np.random.randint(min_dim * 0.003, min_dim * 0.015))
    for i in [0, 1, 2]:
        for j in [0, 1, 2, 3]:
